...
/****** Object:  StoredProcedure [dbo].[sproc_sms_transaction]    Script Date: 8/22/2020 8:47:44 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN  
 ...
 BEGIN  
  set @sql = 'SELECT sms_operator  
   ...
   ,sms_gateway_id  
   ,created_date  
  FROM tbl_sms_transaction   with (NOLOCK)
  where 1=1 '  
 ...
 set @sql =  ' select sms_txn_id, sms_destination_no, message from tbl_sms_transaction with (NOLOCK) where status = ''pending''  '
