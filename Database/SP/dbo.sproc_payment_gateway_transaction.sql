...
/****** Object:  StoredProcedure [dbo].[sproc_payment_gateway_transaction]    Script Date: 9/1/2020 4:02:08 PM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================        
ALTER PROCEDURE [dbo].[sproc_payment_gateway_transaction]
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			--insert notification for load fund  
			DECLARE @bank VARCHAR(50);--bank selection
			...
			VALUES (
				'Load Fund'
				--,nullif(@remarks, 'Fund Loaded From ' + nullif(@bank, '') + ' Bank of Rs ' + @amount)
				,'Fund Loaded From ' + nullif(cast(@bank AS VARCHAR(20)), '') + ' Bank of Rs ' + cast(@amount AS VARCHAR(20))
				,'Load_Fund'
				,'n'
				,cast(@user_id AS VARCHAR(20))
			...
			SET @notiId = @@IDENTITY
			SET @dataPayload = dbo.func_notification_payload(@notiId, @mobile, @amount);
